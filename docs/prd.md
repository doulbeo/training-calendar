...
- 每个动作可查看此前同名动作的历史计划；历史仅包含当前日期之前的计划，按日期倒序展示“日期 组数×次数 重量 · 组数×次数 重量”
...
- 仅点击顶部「训练日历」主动刷新，并清除浏览位置、回到今天。
- 查看期间不因时间经过、切换到后台或重新获得焦点而自动刷新。
- 同一标签页保留当前月份、选中训练日、历史展开状态和滚动位置；页面重新加载或从热身清单返回时恢复。
- 浏览位置保存在会话中，不缓存整份训练计划；浏览器禁止存储时，在当前页面内保持状态。
